...
export function SubmitForm() {
  ...
  if (state === "success") {
    return (
      <div className="border border-[var(--hair)] bg-white p-8">
        <p className="text-lg font-semibold text-[var(--ink)]">收到！审核通过会出现在案例库。</p>
        <p className="mt-2 text-sm leading-7 text-[var(--mute)]">
          我们会逐条人工审核，确认真实可复现后上架。感谢投稿。
        </p>
        <button type="button" className="gc-btn mt-6" onClick={() => setState("idle")}>
          再提交一个
        </button>
      </div>
    );
  ...
  return (
    <form onSubmit={handleSubmit} className="grid gap-5 border border-[var(--hair)] bg-white p-6 md:p-8">
      ...
      {state === "rate-limited" ? (
        <p className="border border-[var(--orange)] bg-[rgba(255,87,51,0.06)] p-3 text-sm text-[var(--ink)]">
          提交太频繁，稍后再试。
        </p>
      ) : null}
      {state === "unavailable" ? (
